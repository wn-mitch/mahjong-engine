...
<main class="mx-auto grid gap-6 p-6 content-start max-sm:p-3 max-sm:gap-3 {layout}">
	...
	<!-- Card-table column: opponents ring the top, the table floats in the vertical center, and
	     the player tray anchors to the bottom of the viewport (desktop). Below lg it is a plain
	     top-to-bottom stack. -->
	<div class="flex flex-col gap-4 min-w-0 lg:min-h-[calc(100vh-9rem)]">
		{#if ended}
			<EndBanner />
		{/if}

		<!-- Opponents ring the top, demoted into the sunk tone. -->
		...
		<!-- Table center: the focal shared state, vertically centered in the slack. The claim
		     prompt sits here because it concerns a tile resting on the table. -->
		<div class="grid gap-4 content-start lg:flex-1 lg:content-center">
			<DiscardRiver />
		...
		<!-- Player tray: hand fused with its turn status and actions, anchored to the bottom. -->
		<PlayerHand />
	</div>
